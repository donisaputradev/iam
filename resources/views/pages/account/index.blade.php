...
@section('main')
    <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
        <div class="max-w-screen-xl px-4 py-4">
            <!-- Start coding here -->
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Daftar Identitas/Akun</h2>
            <div class="bg-white pb-8 dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form class="flex items-center" action="{{ route('account') }}" method="GET">
                                ...
                                @foreach ($users as $item)
                                    <tr class="border-b dark:border-gray-700">
                                        <th scope="row"
                                            ...
                                            <div class="flex justify-start items-center">
                                                ...
                                                <div class="ml-2">
                                                    ...
                                                    <p class="text-gray text-sm">{{ $item->username }}</p>
                                                </div>
                                            </div>
                                        </th>
                                        <td class="px-4 py-3">
                                        ...
                                        <td class="px-4 py-3">
                                            ...
                                            {{ $item->desa != null ? $item->desa . ',' : '' }}
                                            {{ $item->kecamatan != null ? $item->kecamatan . ',' : '' }}
                                            {{ $item->kabupaten != null ? $item->kabupaten . ',' : '' }}
                                            {{ $item->provinsi ?? '' }}
                                        </td>
                                        <td class="px-4 py-3">
                                            <span
    ...
    @include('pages.account.modals.add')

    @include('pages.account.modals.delete')
@endsection
